...
class MySquadComponent extends Component {
  ...
  addSquad = () => {
    ...
    parameter.append("event_id", this.props.route.params.event_id);
    parameter.append("invite_squad", this.state.arrSelectedUserId.join(","));
    parameter.append("invite_user", "");

    axios
      .post("http://squadvibes.onismsolution.com/api/addMembers", parameter)
      .then((res) => {
        console.log("API RES", res.data);
        Toast.show("Request successfully sent.");
  ...
  render() {
    return (
      <View style={{ flex: 1, backgroundColor: colors.white }}>
        ...
        <TouchableOpacity activeOpacity={0.8} onPress={this.addSquad}>
          <LinearGradient
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
