...
class AddPeopleComponent extends Component {
  ...
  componentDidMount() {
    this._unsubscribe = this.props.navigation.addListener('focus', () => {
      ...
      if (this.props.route.params != undefined) {
        ...
        const { isFromMySquad, group_member, squad_id, title, isFromAddFriend, friends, isFromContact } = this.props.route.params;
        ...
        if (this.props.route.params.hasOwnProperty('group_member') && group_member.length > 0) {
          var isExist = this.state.data.filter(data => data.id == 4)
          if (isExist.length > 0) {
  ...
  onPressOption = index => {
    if (index === 0) {
      ...
      // this.setState({ show_add_by_user_id_popup: !this.state.show_add_by_user_id_popup })
      this.props.navigation.navigate('AddByUserId', {
        isFromAddFriend: this.state.isFromAddFriend,
        friends: this.state.friends
      });
    }else if(index == 3){
